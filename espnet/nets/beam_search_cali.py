...
class Hypothesis(NamedTuple):
    ...
    confidence_ctc: torch.Tensor = None
    ...
    def asdict(self) -> dict:
        ...
        return self._replace(
            ...
            confidence_ctc=self.confidence_ctc.tolist(),
        )._asdict()
...
class BeamSearch(torch.nn.Module):
    ...
    def init_hyp(self, x: torch.Tensor, maxlen=None) -> List[Hypothesis]:
        ...
        conf_tmp = torch.ones(maxlen).to(torch.long)
        conf_tmp = torch.nn.functional.one_hot(conf_tmp, num_classes=self.n_vocab).to(torch.float)
    ...
    def search(
        ...
        for hyp in running_hyps:
            ...
            for k in self.full_scorers:
                weighted_scores += torch.log_softmax(self.weights[k]*scores[k], dim=-1)
            ...
            if self.do_pre_beam:
                ...
                part_ids = torch.tensor(range(self.n_vocab))

            part_scores, part_states = self.score_partial(hyp, part_ids, x)
            for k in self.part_scorers:
                weighted_scores[part_ids] += torch.log_softmax(self.weights[k]*part_scores[k], dim=-1)
            ...
            for j, part_j in zip(*self.beam(weighted_scores, part_ids)):
                ...
                # # update confidence
                time=len(hyp.yseq)-1
                new_confidence = hyp.confidence.clone()
                new_confidence[time] = torch.softmax(scores['decoder']*self.weights['decoder'],dim=-1)
                new_confidence_ctc = hyp.confidence_ctc.clone()
                new_confidence_ctc[time] = torch.softmax(part_scores['ctc']*self.weights['ctc'],dim=-1)
                ...
                best_hyps.append(
                    Hypothesis(
                        ...
                        confidence_ctc=new_confidence_ctc,
                    )
                )
    ...
    def forward(
        ...
        running_hyps = self.init_hyp(x if pre_x is None else pre_x, maxlen=maxlen)
        # running_hyps = self.init_hyp(x, maxlen=maxlen)
        ended_hyps = []
