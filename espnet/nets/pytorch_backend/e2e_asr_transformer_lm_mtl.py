...
from espnet.nets.pytorch_backend.transformer.decoder_lm_mtl import Decoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.batch_ratio_lm=args.batch_ratio_lm
    ...
    def forward(self, xs_pad, ilens, ys_pad, epoch=None, corpus=None):
        """E2E forward.
        ...
        if self.decoder is not None:
            ...
            # scheduled sampling
            if self.scheduled_sampling > 0 and self.training:
                threshold = (epoch / self.max_epoch) * self.scheduled_sampling
                if print_option:
                    print('ys_in_pad before:', ys_in_pad[0])
                with torch.no_grad():
                    lm_hyps, _ = self.decoder(ys_in_pad, ys_mask, 0, 0, txt_only=True)
                    lm_hyps = torch.argmax(lm_hyps, dim=-1).to(ys_in_pad.dtype).detach()

                    assert lm_hyps.size() == ys_in_pad.size()

                    mask = (torch.rand(lm_hyps.size()) < threshold).to(ys_in_pad.dtype).to(ys_in_pad.device)[:,:-1]
                    pad = (ys_in_pad == self.eos).to(ys_in_pad.dtype).to(ys_in_pad.device)
                    
                    # sampling
                    ys_in_pad[:,:-1] = lm_hyps[:,1:] * mask + ys_in_pad[:,:-1] * (1-mask)

                    # padding
                    ys_in_pad = ys_in_pad * (1-pad) + (torch.ones(ys_in_pad.size()) * self.eos).to(ys_in_pad.dtype).to(ys_in_pad.device) * pad

                if print_option:
                    print('ys_in_pad after:', ys_in_pad[0])
            ...
            if corpus is not None:
                # 5. forward LM
                corp_in_pad, corp_out_pad = add_sos_eos(
                    corpus, self.sos, self.eos, self.ignore_id
                )
                corpus_mask = target_mask(corp_in_pad, self.ignore_id)
                pred_pad_cor, _ = self.decoder(corp_in_pad, corpus_mask, 0, 0, txt_only=True)
                
                # 6. compute LM loss
                loss_lm = self.criterion(pred_pad_cor, corp_out_pad)
                self.acc_lm = th_accuracy(
                    pred_pad_cor.view(-1, self.odim), corp_out_pad, ignore_label=self.ignore_id
                )

                # 7. Multi-task learning loss
                loss_att = loss_att + loss_lm

                # 8. check
                if print_option:
                    print('corp_in_pad:', corp_in_pad[0])
                    print('corp_out_pad:', corp_out_pad[0])
                    print('pred_pad_cor:', torch.argmax(pred_pad_cor, dim=-1)[0])
                    print('self.ignore_id:', self.ignore_id)
                    print('self.acc_lm:', self.acc_lm, 'self.acc:', self.acc)
                    print('loss_lm:', loss_lm, 'loss_total:', loss_att, 'self.batch_ratio_lm:', self.batch_ratio_lm)
        else:
            loss_att = None
            self.acc = None
