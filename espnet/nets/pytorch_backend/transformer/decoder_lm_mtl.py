...
from espnet.nets.pytorch_backend.transformer.decoder_layer_lm_mtl import DecoderLayer
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, txt_only=False):
        """Forward decoder.
        ...
        # set forward mode (if txt_only: LM, else: ASR)
        for _, layer in enumerate(self.decoders):
            layer.set_forward_mode(txt_only)

        # forward decoder
        if txt_only:
            x, tgt_mask, _1, _2 = self.decoders(
                x, tgt_mask, 0, 0
            )
        else:
