...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        corpus_iter = self.get_iterator("corpus")
        optimizer = self.get_optimizer("main")
        epoch = train_iter.epoch

        # Get the next batch (a list of json files)
        batch = train_iter.next()
        batch_corpus = corpus_iter.next()
        # self.iteration += 1 # Increase may result in early report,
        # which is done in other place automatically.
        x = _recursive_to(batch, self.device)
        x_corpus = _recursive_to(batch_corpus, self.device)
        _1, _1, corpus = x_corpus
        ...
        if self.ngpu == 0 or self.use_ddp:
            ...
            # apex does not support torch.nn.DataParallel
            loss = (
                data_parallel(self.model, (*x, epoch, corpus), range(self.ngpu)).mean() / self.accum_grad
            )
        if self.use_apex:
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if len(batch[0]) == 2:
            ...
            xs = batch[0][0]
            # b_s,t_s = ys.shape
            # xs = np.zeros(b_s, t_s, 83)
        
        # batch:', ([array([265, 187,  89,  95,  34, 144, 258,   9,  87,  51, 127, 248,  68,
        #         24,  51, 258,  90,  47, 230, 275, 212, 127, 137, 191, 152, 285,
        #         93,  38,  38,  66, 235, 228, 265, 287, 265, 187,  79,  36, 228,

        # batch original: ([array([[-0.5498313 , -0.21169235, -0.3172271 , ...,  1.1130517 ,
        #         -0.32293347,  0.02167536],
        #     [-0.8120726 , -0.8219318 , -0.44226822, ...,  1.154574  ,
...
def train(args):
    ...
    with open(args.valid_json, "rb") as f:
        valid_json = json.load(f)["utts"]
    with open(args.corpus_json, "rb") as f:
        corpus_json = json.load(f)["utts"]
    ...
    corpus = make_batchset(
        corpus_json,
        args.batch_size * args.batch_ratio_lm,
        args.maxlen_in,
        args.maxlen_out,
        args.minibatches,
        min_batch_size=min_batch_size,
    ...
    load_tr_corpus = LoadInputsAndTargets(
        mode="asr",
        load_input=False,
        load_output=True,
        preprocess_conf=args.preprocess_conf,
    ...
    corpus_ds = TransformDataset(corpus, Transform(converter, load_tr_corpus))
    train_sampler = None
    val_sampler = None
    corpus_sampler = None
    ...
    if args.use_ddp:
        ...
        corpus_sampler = DistributedSampler(corpus_ds)
        shuffle = False
    ...
    updater = CustomUpdater(
        model,
        args.grad_clip,
        {"main": train_iter, "corpus":corpus_iter},
        optimizer,
        device,
        args.ngpu,
        args.grad_noise,
