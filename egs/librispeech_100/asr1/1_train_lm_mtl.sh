...
train_config=conf/tuning/train_conformer_ctcatt_wo_lsm_lm_mtl.yaml
lm_config=conf/tuning/lm_transformer.yaml
decode_config=conf/tuning/decode_ctcatt.yaml
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # get shortest 40M sentences from Libri Corpus
    awk '{ print length(), $0 | "sort -n" }' $lmdatadir/train.txt | cut -d " " -f 2- | head -40000000 > $lmdatadir/train_40M_short.txt
    
    # prepare LibriCorpus in json format for LM-AM joint training
    corpus_names="train_40M_short.txt"
    for corpus_name in ${corpus_names}; do
        corpus_name_new=$corpus_name.int
        ./utils/sym2int.pl --map-oov "<unk>" $dict < $lmdatadir/$corpus_name > $lmdatadir/$corpus_name_new
        python local/data2json_lm2_compress.py $lmdatadir/$corpus_name $lmdatadir/$corpus_name_new ${feat_tr_dir}/data_${bpemode}${nbpe}.json
    done
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --corpus-json data/local/lm_train_${bpemode}${nbpe}/train_40M_short.txt.json
fi
